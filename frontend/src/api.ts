...
export async function getProviderInfo(): Promise<ProviderInfo> {
  try {
    const res = await fetch(`${API_BASE_URL}/api/provider-info`);
    await handleResponse(res);
    return await res.json();
  } catch (error) {
...
export async function textToSpeech(
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/voice/text-to-speech`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text,
...
export async function voiceChat(
  ...
  try {
    ...
    formData.append('audio', audioBlob, 'audio.webm');
    formData.append('language', language);
    formData.append('auto_speak', autoSpeak.toString());
    ...
    const res = await fetch(`${API_BASE_URL}/api/voice/voice-chat`, {
      method: 'POST',
      body: formData
    });
    ...
    if (contentType?.includes('audio/')) {
      ...
      (audioBlob as any).metadata = {
        transcript,
        ai_response: aiResponse,
        conversation_id: parseInt(convId) || conversationId,
